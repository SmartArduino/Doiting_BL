...
bfl_err_t bfl_ble_mesh_register_generic_client_callback(bfl_ble_mesh_generic_client_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_GENERIC_CLIENT, callback) == 0 ? BFL_OK : BFL_FAIL);
}

static bool generic_client_get_need_param(bfl_ble_mesh_opcode_t opcode)
{
    switch (opcode) {
...
bfl_err_t bfl_ble_mesh_register_generic_server_callback(bfl_ble_mesh_generic_server_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_GENERIC_SERVER, callback) == 0 ? BFL_OK : BFL_FAIL);
}
