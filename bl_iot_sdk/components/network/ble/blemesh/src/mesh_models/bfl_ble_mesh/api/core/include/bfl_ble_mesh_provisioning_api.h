...
#ifndef _BFL_BLE_MESH_PROVISIONING_API_H_
#define _BFL_BLE_MESH_PROVISIONING_API_H_
...
typedef void (* bfl_ble_mesh_prov_cb_t)(bfl_ble_mesh_prov_cb_event_t event,
 ...
 *
 */
bfl_err_t bfl_ble_mesh_register_prov_callback(bfl_ble_mesh_prov_cb_t callback);
 ...
 *
 */
bool bfl_ble_mesh_node_is_provisioned(void);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_node_prov_enable(bfl_ble_mesh_prov_bearer_t bearers);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_node_prov_disable(bfl_ble_mesh_prov_bearer_t bearers);
 ...
 * @return       BFL_OK on success or error code otherwise.
 */
bfl_err_t bfl_ble_mesh_node_set_oob_pub_key(uint8_t pub_key_x[32], uint8_t pub_key_y[32],
        uint8_t private_key[32]);

/**
 * @brief        Provide provisioning input OOB number.
 *
 * @note         This is intended to be called if the user has received BFL_BLE_MESH_NODE_PROV_INPUT_EVT
 *               with BFL_BLE_MESH_ENTER_NUMBER as the action.
 *
 ...
 *
 */
bfl_err_t bfl_ble_mesh_node_input_number(uint32_t number);

/**
 * @brief        Provide provisioning input OOB string.
 *
 * @note         This is intended to be called if the user has received BFL_BLE_MESH_NODE_PROV_INPUT_EVT
 *               with BFL_BLE_MESH_ENTER_STRING as the action.
 *
 ...
 *
 */
bfl_err_t bfl_ble_mesh_node_input_string(const char *string);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_set_unprovisioned_device_name(const char *name);
 ...
 * @return       BFL_OK on success or error code otherwise.
 */
bfl_err_t bfl_ble_mesh_provisioner_read_oob_pub_key(uint8_t link_idx, uint8_t pub_key_x[32],
        uint8_t pub_key_y[32]);

/**
 * @brief        Provide provisioning input OOB string.
 *
 *               This is intended to be called after the bfl_ble_mesh_prov_t prov_input_num
 *               callback has been called with BFL_BLE_MESH_ENTER_STRING as the action.
 *
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_input_string(const char *string, uint8_t link_idx);

/**
 * @brief        Provide provisioning input OOB number.
 *
 *               This is intended to be called after the bfl_ble_mesh_prov_t prov_input_num
 *               callback has been called with BFL_BLE_MESH_ENTER_NUMBER as the action.
 *
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_input_number(uint32_t number, uint8_t link_idx);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_prov_enable(bfl_ble_mesh_prov_bearer_t bearers);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_prov_disable(bfl_ble_mesh_prov_bearer_t bearers);
 ...
 *                    BFL_BLE_MESH_PROV_ADV and BFL_BLE_MESH_PROV_GATT enabled, and cannot set flags
 ...
 *                  bfl_ble_mesh_register_prov_callback. And in the callback, the Provisioner
 ...
 *                    device address can be added, bearers shall be set with BFL_BLE_MESH_PROV_ADV
 ...
 *                    address need to be added, bearers shall be set with BFL_BLE_MESH_PROV_GATT enabled,
 ...
 *                    of BFL_BLE_MESH_PROV_ADV and BFL_BLE_MESH_PROV_GATT enabled (recommend to enable the
 ...
 *                  - Note: BFL_BLE_MESH_PROV_ADV, BFL_BLE_MESH_PROV_GATT and ADD_DEV_START_PROV_NOW_FLAG
 *                          can not be enabled at the same time.
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_add_unprov_dev(bfl_ble_mesh_unprov_dev_add_t *add_dev,
 ...
 *
 *  @note: 1. Currently address type only supports public address and static random address.
 *         2. Bearer must be equal to BFL_BLE_MESH_PROV_ADV or BFL_BLE_MESH_PROV_GATT, since
 ...
 *            of the BFL_BLE_MESH_PROVISIONER_RECV_UNPROV_ADV_PKT_EVT event.
 ...
 *         4. Recommend to use only one of the functions "bfl_ble_mesh_provisioner_add_unprov_dev"
 *            and "bfl_ble_mesh_provisioner_prov_device_with_addr" by a Provisioner.
 */
bfl_err_t bfl_ble_mesh_provisioner_prov_device_with_addr(const uint8_t uuid[16],
            bfl_ble_mesh_bd_addr_t addr, bfl_ble_mesh_addr_type_t addr_type,
            bfl_ble_mesh_prov_bearer_t bearer, uint16_t oob_info, uint16_t unicast_addr);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_delete_dev(bfl_ble_mesh_device_delete_t *del_dev);
 ...
 *
 */
typedef void (*bfl_ble_mesh_prov_adv_cb_t)(const bfl_ble_mesh_bd_addr_t addr, const bfl_ble_mesh_addr_type_t addr_type,
        const uint8_t adv_type, const uint8_t *dev_uuid,
        uint16_t oob_info, bfl_ble_mesh_prov_bearer_t bearer);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_set_dev_uuid_match(const uint8_t *match_val, uint8_t match_len,
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_set_prov_data_info(bfl_ble_mesh_prov_data_info_t *prov_data_info);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_set_static_oob_value(const uint8_t *value, uint8_t length);
 ...
 *                bfl_ble_mesh_prov_t will be ignored, and Provisioner will use this address as its
 ...
 *
 */
bfl_err_t bfl_ble_mesh_provisioner_set_primary_elem_addr(uint16_t addr);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_set_fast_prov_info(bfl_ble_mesh_fast_prov_info_t *fast_prov_info);
 ...
 *
 */
bfl_err_t bfl_ble_mesh_set_fast_prov_action(bfl_ble_mesh_fast_prov_action_t action);

#ifdef __cplusplus
}
#endif

#endif /* _BFL_BLE_MESH_PROVISIONING_API_H_ */
