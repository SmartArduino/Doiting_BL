...
// Mapping between BFL_LOG_LEVEL and BT_TRACE_LEVEL
...
#define BT_PRINT_E(tag, format, ...)   {bfl_log_write(BFL_LOG_ERROR,   tag, LOG_FORMAT(E, format), bfl_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_W(tag, format, ...)   {bfl_log_write(BFL_LOG_WARN,    tag, LOG_FORMAT(W, format), bfl_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_I(tag, format, ...)   {bfl_log_write(BFL_LOG_INFO,    tag, LOG_FORMAT(I, format), bfl_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_D(tag, format, ...)   {bfl_log_write(BFL_LOG_DEBUG,   tag, LOG_FORMAT(D, format), bfl_log_timestamp(), tag, ##__VA_ARGS__); }
#define BT_PRINT_V(tag, format, ...)   {bfl_log_write(BFL_LOG_VERBOSE, tag, LOG_FORMAT(V, format), bfl_log_timestamp(), tag, ##__VA_ARGS__); }
...
/* relate to BFL_BT_STATUS_xxx in bfl_bt_defs.h */
typedef enum {
    BT_STATUS_SUCCESS = 0,
    BT_STATUS_FAIL,
    BT_STATUS_NOT_READY,
